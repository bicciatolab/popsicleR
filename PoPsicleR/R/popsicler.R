...
DTP <- function(data, markers, annotation){
  ...
  print(DotPlot(object=data, features=markers.to.plot, cols=c("white", "red"),
...
ApplyRegression <- function(UMI, variables='none', explore_PC=FALSE,  out_folder=getwd()){
  ...
  popsicler:::four_plots(UMI, "pca")
  popsicler:::four_plots(UMI, "tsne")
  popsicler:::four_plots(UMI, "umap")
  invisible(dev.off())
...
CalculateCluster <- function(UMI, dim_pca, organism="human", marker.list='none', PCA=TRUE, cluster_res=0.8, out_folder=getwd()){
  ...
  for(algo in algos){
    popsicler:::algo_plot(Cluster_dir, UMI, algo, "04a_clusters" , cluster_res)
  }
  if ("doublets" %in% colnames(UMI@meta.data)){
    popsicler:::FTP(UMI, Cluster_dir, "04c_", "umap", 10, to_be_plotted=c("doublets","doublets_score"), "_doublets_umap")
    popsicler:::FTP(UMI, Cluster_dir, "04c_", "tsne", 10, to_be_plotted=c("doublets","doublets_score"), "_doublets_tsne")
  }
  ...
  top.markers.2 <- umi.markers %>% dplyr::group_by(cluster) %>% dplyr::top_n(n=2, wt=FC_col)
  ...
  for(cluster in unique(top.markers.2$cluster)) {
    ...
    #vg1 <- popsicler:::VLN(UMI, features=genes[1], pt.size=FALSE, ncol=1) + ggplot2::geom_boxplot(width=0.1, outlier.shape=NA) + NoLegend() + ggplot2::xlab("")
  ...
  ### expression heatmap
  top.markers.10 <- umi.markers %>% dplyr::group_by(cluster) %>% dplyr::top_n(n = 10, wt = FC_col)
  ...
  cat(bold(green("Plotting Markers list graphs for each cluster \n")))
  popsicler:::FTP(UMI, Cluster_dir, "04d_", "umap", ftp_h, unlist(marker.list), "_markers")
  popsicler:::FTP(UMI, Cluster_dir, "04d_", "tsne", ftp_h, unlist(marker.list), "_markers")
  ### visualize common markers by violin plots
  pdf(paste0(Cluster_dir, "/04e_Violin_markers.pdf"), height = ftp_h, useDingbats=FALSE)
  print(popsicler:::VLN(UMI, 10, unlist(marker.list)))
  dev.off()
  ...
  pdf(paste0(Cluster_dir, "04f_Dotplot_markers.per.cluster.pdf"), width=width_sig, height=10, useDingbats=FALSE)
  print(popsicler:::DTP(UMI, marker.list, "seurat_clusters"))
  invisible(dev.off())
...
MakeAnnotation <- function(UMI, organism, marker.list='none', thresh=20, out_folder=getwd()){
  ...
  if(organism == 'human'){
    ...
    UMI <- popsicler:::SR_plots("hpca", hpca.se, UMI, Annot_dir)
    UMI <- popsicler:::SR_plots("BpEn", BpEn.se, UMI, Annot_dir)
    ...
    for(single_annot in annotations){
      ...
      pdf(paste0(Annot_dir, paste0("/05d_DotPlot_Immune_Markers_", single_annot, "ImmGen.pdf")), width=width_sig, height=10, useDingbats=FALSE)
    ...
    UMI <- popsicler:::SR_plots("ImmGen", Ig.se, UMI, Annot_dir)
    ### custom DotPlot for immune markers
    pdf(paste0(Annot_dir, "/05d_DotPlot_Immune_Markers_ImmGen.pdf"), width=width_sig, height=10, useDingbats=FALSE)
    popsicler:::DTP(UMI, marker.list, 'ImmGen.sc.main.labels')
    invisible(dev.off())
    ## Custom DotPlot for immune markers
